(function () {
  ...
  const renderMembers = () => {
    ...
    if (guard) guard.hidden = !permissionMode || permissionUnlocked;
    const keyword = getKeyword();
    ...
    filtered.forEach((item) => {
      ...
      if (permissionMode && permissionUnlocked) {
        ...
        ["customer", "admin"].forEach((role) => {
          ...
          option.selected = role === (item.role || "customer");
  ...
  const updateMemberRole = async (event) => {
    ...
    const previousRole = member?.role || "customer";
    ...
    try {
      ...
      setPermissionMessage("권한이 변경되었습니다.", "success");
      renderMembers();
    } catch (error) {
      console.error(error);
      select.value = previousRole;
      setPermissionMessage(error.message || "권한 변경 중 오류가 발생했습니다.");
    } finally {
