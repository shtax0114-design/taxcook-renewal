(function () {
  ...
  const savePendingProfile = (profile) => {
    ...
    core.writeSessionProfile?.(profile);
  };
  ...
  const authErrorMessage = (error) => {
    ...
    if (message.includes("Email not confirmed")) return "이메일 확인 대기 상태입니다. Supabase Auth 설정 또는 사용자 확인 상태를 확인해야 합니다.";
    ...
    if (message) return `Supabase 오류${status}: ${message}`;
  ...
  document.querySelector("[data-signup-form]")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (user) core.writeSessionProfile?.({ id: user.id, uid: user.id, ...profile });
  ...
  document.querySelector("[data-login-form]")?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (authData.user) {
        core.writeSessionProfile?.({
          id: authData.user.id,
          uid: authData.user.id,
          ...(pendingProfile || core.fallbackProfile(authData.user))
        });
      }
