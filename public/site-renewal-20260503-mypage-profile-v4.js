(function () {
  ...
  const syncBizNumberInput = () => {
    const businessType = document.querySelector('[name="businessType"]');
    const bizNumber = document.querySelector('[name="bizNumber"]');
    if (!businessType || !bizNumber) return;
    const enabled = businessType.value === "사업자";
    bizNumber.disabled = !enabled;
    if (!enabled) bizNumber.value = "";
  };

  const syncPasswordLink = (user) => {
    const link = document.querySelector(".profile-password-link");
    if (!link) return;
    ...
    link.hidden = !providers.has("email");
  };
  ...
  const formatError = (error) => {
    ...
    if (!parts.length) return "고객정보 저장 중 오류가 발생했습니다.";
    return `고객정보 저장 실패: ${parts.join(" / ")}`;
  };
  ...
  document.querySelector('[name="businessType"]')?.addEventListener("change", syncBizNumberInput);
  document.querySelector('[name="bizNumber"]')?.addEventListener("input", (event) => {
    event.target.value = core.onlyDigits(event.target.value).slice(0, 10);
  });
  syncBizNumberInput();
  ...
  document.querySelector("[data-profile-form]")?.addEventListener("submit", async (event) => {
    ...
    const profile = {
      ...
      biz_number: businessType === "사업자" ? core.onlyDigits(formData.get("bizNumber")) : ""
    };
    ...
    try {
      ...
      core.showMessage("[data-profile-message]", formatError(error));
    } finally {
