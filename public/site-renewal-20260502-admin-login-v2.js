(function () {
  ...
  document.querySelector("[data-admin-login-form]")?.addEventListener("submit", async (event) => {
    ...
    const email = loginId.includes("@") ? core.normalizeEmail(loginId) : core.userIdToEmail(loginId);
    const password = String(formData.get("password") || "");

    showMessage("");
    if (!loginId) {
      showMessage("로그인ID를 입력해 주세요.");
      return;
    }
    if (loginId.includes("@") && !core.isValidEmail(email)) {
      showMessage("이메일 형식의 로그인ID를 확인해 주세요.");
      return;
    }
    if (!loginId.includes("@") && !core.isValidUserId(loginId)) {
      showMessage("로그인ID는 영문 소문자, 숫자, 점, 밑줄, 하이픈 4~30자로 입력해 주세요.");
      return;
    }
    if (!password) {
