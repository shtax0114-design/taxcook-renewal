...
if (membersBody) {
  const renderMembers = () => {
    ...
    if (!members.length) {
      membersBody.innerHTML = '<tr><td colspan="7" class="empty-cell">아직 가입된 회원이 없습니다.</td></tr>';
      return;
    }
    ...
    members.forEach((member) => {
      ...
      appendCell(row, member.birth);
      appendCell(row, member.businessType);
      appendCell(row, member.bizNumber || "-");

      const actionCell = document.createElement("td");
      const deleteButton = document.createElement("button");
      deleteButton.type = "button";
      deleteButton.className = "table-button";
      deleteButton.dataset.deleteMember = member.userId;
