...
const readMembers = () => {
  try {
    return JSON.parse(localStorage.getItem(storeKey) || "[]").map((member) => ({
      authority: member.authority || "고객",
      accountType: member.accountType || "비밀번호",
      loginId: member.loginId || member.userId || "",
      password: member.password || "",
      name: member.name || "",
      phone: member.phone || "",
      updatedBy: member.updatedBy || member.name || "",
      updatedAt: member.updatedAt || member.createdAt || new Date().toISOString(),
      userId: member.userId || member.loginId || "",
      birth: member.birth || "",
      businessType: member.businessType || "",
      bizNumber: member.bizNumber || "",
      createdAt: member.createdAt || member.updatedAt || new Date().toISOString()
    }));
  } catch (error) {
    return [];
  }
...
if (signupForm) {
  signupForm.addEventListener("submit", (event) => {
    ...
    if (members.some((member) => member.loginId === userId || member.userId === userId)) return showAuthMessage("이미 가입된 아이디입니다.");
...
if (loginForm) {
  loginForm.addEventListener("submit", (event) => {
    ...
    const userId = String(data.get("userId") || "").trim();
    const password = String(data.get("password") || "");
    const member = readMembers().find((item) => (item.loginId === userId || item.userId === userId) && item.password === password);

    if (!member) return showAuthMessage("아이디 또는 비밀번호를 확인해주세요.");

    localStorage.setItem(sessionKey, member.loginId || userId);
...
if (membersBody) {
  ...
  const renderMembers = () => {
    ...
    members.forEach((member) => {
      ...
      checkbox.dataset.memberSelect = member.loginId;
      checkbox.setAttribute("aria-label", `${member.loginId} 선택`);
      ...
      editLink.href = `signup.html?edit=${encodeURIComponent(member.loginId)}`;
      editLink.className = "edit-link";
      editLink.setAttribute("aria-label", `${member.loginId} 수정`);
      editLink.textContent = "∕";
      editCell.appendChild(editLink);
      row.appendChild(editCell);

      appendCell(row, member.authority);
      appendCell(row, member.accountType);
      appendCell(row, member.loginId);
      appendCell(row, "****");
  ...
  membersBody.addEventListener("click", (event) => {
    ...
    const members = readMembers().filter((member) => member.loginId !== userId);
    writeMembers(members);
