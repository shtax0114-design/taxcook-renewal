...
const legacyMemberStoreKey = "taxcookPreviewMembers";
...
const readLegacyMembers = () => {
  try {
    return JSON.parse(localStorage.getItem(legacyMemberStoreKey) || "[]");
  } catch (error) {
    return [];
  }
};
...
if (loginForm) {
  loginForm.addEventListener("submit", async (event) => {
    ...
    const migrateLegacyMember = async () => {
      const legacyMember = readLegacyMembers().find((member) => {
        return String(member.userId || "").toLowerCase() === userId && member.password === password;
      });

      if (!legacyMember) return false;
      ...
      const profile = {
        ...
        birth: legacyMember.birth || "",
        businessType: legacyMember.businessType || "",
        bizNumber: legacyMember.bizNumber || ""
      };
      ...
      await saveProfile(services.db, user, profile);
      showAuthMessage("기존 회원 정보를 연결했습니다. My 택스쿡으로 이동합니다.", "success");
      window.setTimeout(() => {
        window.location.href = "mypage.html";
      }, 600);
      return true;
    ...
    try {
      ...
      try {
        const migrated = await migrateLegacyMember();
        if (!migrated) showAuthMessage(translateAuthError(error));
      } catch (migrationError) {
        showAuthMessage(translateAuthError(migrationError));
      }
    } finally {
