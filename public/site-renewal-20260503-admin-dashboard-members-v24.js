(function () {
  ...
  const resizeDetailFrame = (frame) => {
    try {
      const doc = frame.contentDocument || frame.contentWindow?.document;
      if (!doc) return;
      const visibleBlocks = Array.from(doc.querySelectorAll(".legacy-section, .service-detail-form, .developer-log-section"))
        .filter((element) => !element.hidden && element.offsetParent !== null);
      const bottom = visibleBlocks.reduce((max, element) => {
        const rect = element.getBoundingClientRect();
        return Math.max(max, rect.bottom + (doc.defaultView?.scrollY || 0));
      }, 0);
      const height = bottom || Math.min(
        doc.body?.scrollHeight || 0,
        doc.documentElement?.scrollHeight || 0
      );
      if (height > 0) frame.style.height = `${Math.max(320, height + 8)}px`;
    } catch (error) {
  ...
  const selectedMemberIds = () =>
    Array.from(document.querySelectorAll("[data-member-check]:checked")).map((input) => input.value);

  const syncMemberDeleteAccess = () => {
    const button = document.querySelector("[data-member-delete-selected]");
    if (!button) return;
    const isDeveloper = adminProfile?.role === "developer";
    button.disabled = !isDeveloper;
    button.title = isDeveloper ? "" : "회원 삭제는 developer만 가능합니다.";
  };
  ...
  const renderApplications = () => {
    ...
    filtered.forEach((item) => {
      ...
      if (expandedApplicationId === item.id) {
        ...
        frame.src = `application-detail.html?id=${encodeURIComponent(item.id)}&detailVersion=20260503-width-v3`;
        frame.addEventListener("load", () => {
          try {
            frame._detailResizeObserver?.disconnect();
            const doc = frame.contentDocument || frame.contentWindow?.document;
            const observer = new ResizeObserver(() => resizeDetailFrame(frame));
            if (doc?.body) observer.observe(doc.body);
            doc?.querySelectorAll(".legacy-section, .service-detail-form, .developer-log-section").forEach((element) => observer.observe(element));
            frame._detailResizeObserver = observer;
          } catch (error) {
            frame._detailResizeObserver = null;
          }
          resizeDetailFrame(frame);
          window.setTimeout(() => resizeDetailFrame(frame), 120);
          window.setTimeout(() => resizeDetailFrame(frame), 450);
          window.setTimeout(() => resizeDetailFrame(frame), 1000);
          window.setTimeout(() => resizeDetailFrame(frame), 2000);
        });
  ...
  const saveMemberProfile = async (event) => {
    ...
    try {
      if (isAdd) {
        ...
        members = members.map((item) => item.id === id ? {
          ...item,
          ...payload,
  ...
  const deleteSelectedMembers = async () => {
    if (adminProfile?.role !== "developer") {
      setMemberMessage("회원 삭제는 developer만 가능합니다.");
      return;
    }
    const ids = selectedMemberIds();
    if (!ids.length) {
      setMemberMessage("삭제할 회원을 선택해주세요.");
      return;
    }
    const targets = members.filter((item) => ids.includes(item.id));
    const protectedTargets = targets.filter((item) =>
      item.id === adminProfile?.id || normalize(item.user_id) === protectedDeveloperEmail
    );
    if (protectedTargets.length) {
      setMemberMessage("고정 developer 또는 본인 계정은 삭제할 수 없습니다.");
      return;
    }
    const names = targets.map((item) => item.name || item.user_id || item.id).join(", ");
    if (!confirm(`선택한 회원 ${ids.length}명을 삭제할까요?\n\n삭제 시 Supabase Auth 계정과 관련 신청내역도 함께 삭제될 수 있습니다.\n${names}`)) return;

    const button = document.querySelector("[data-member-delete-selected]");
    core.setBusy(button, true, "삭제 중...");
    setMemberMessage("");
    try {
      const { data: sessionData } = await core.getClient().auth.getSession();
      const accessToken = sessionData?.session?.access_token;
      if (!accessToken) throw new Error("관리자 로그인이 필요합니다.");
      const response = await fetch(`${window.TAXCOOK_SUPABASE.url}/functions/v1/admin-delete-members`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${accessToken}`,
          "Content-Type": "application/json"
      ...
      if (!response.ok && response.status !== 207) {
        throw new Error(result.error || "회원 삭제 중 오류가 발생했습니다.");
      }
      const deletedIds = result.deletedIds || [];
      members = members.filter((item) => !deletedIds.includes(item.id));
      const checkAll = document.querySelector("[data-member-check-all]");
      if (checkAll) checkAll.checked = false;
      renderMembers();
      const skippedText = result.skipped ? ` ${result.skipped}명은 보호 계정이라 건너뛰었습니다.` : "";
      const partialText = result.errors?.length ? " 일부 회원은 삭제하지 못했습니다." : "";
      setMemberMessage(`${deletedIds.length}명을 삭제했습니다.${skippedText}${partialText}`, result.errors?.length ? "error" : "success");
    } catch (error) {
      console.error(error);
      setMemberMessage(error.message || "회원 삭제 중 오류가 발생했습니다.");
    } finally {
  ...
  document.querySelector("[data-member-delete-selected]")?.addEventListener("click", deleteSelectedMembers);
