(function () {
  ...
  const render = (application) => {
    ...
    const vataxAmount = Math.max(baseFee - discountAmount, 0);
    const finalPaymentAmount = isGitax ? application.final_payment_amount ?? application.total_amount ?? baseFee : application.final_payment_amount ?? application.total_amount ?? vataxAmount;
    const supplyAmount = isGitax ? application.supply_amount ?? application.total_amount ?? finalPaymentAmount : application.supply_amount ?? application.total_amount ?? finalPaymentAmount;
  ...
  const buildPayload = () => {
    ...
    if (currentApplication?.service === "gitax") {
      ...
      const finalAmount = Math.max(fee - discountAmount, 0);
      Object.assign(payload, {
        base_fee: fee,
