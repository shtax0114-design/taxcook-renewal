(function () {
  ...
  const openMemberModal = (memberId = "") => {
    ...
    if (!member) {
      setMemberMessage("신규 회원 추가는 로그인 계정 생성 기능 연결 후 사용할 수 있습니다.");
      return;
    }
    form.reset();
    ...
    document.querySelector("[data-member-form-title]").textContent = "회원정보 수정";
    modal.hidden = false;
  ...
  const saveMemberProfile = async (event) => {
    ...
    const payload = {
      user_id: String(form.elements.userId.value || "").trim().toLowerCase(),
    ...
    try {
      ...
      members = members.map((item) => item.id === id ? data : item);
      closeMemberModal();
      setMemberMessage("회원정보가 저장되었습니다.", "success");
      renderMembers();
    } catch (error) {
      console.error(error);
  ...
  const downloadMembers = () => {
    ...
    link.href = url;
    link.download = `taxcook-members-${todayText()}.csv`;
