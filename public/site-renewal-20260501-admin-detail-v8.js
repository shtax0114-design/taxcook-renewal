(function () {
  ...
  const render = (application) => {
    ...
    const baseFee = isGitax ? application.base_fee ?? defaultGitaxFee(reportType) : defaultVataxFee(application.business_type);
    const finalPaymentAmount = isGitax ? application.final_payment_amount ?? application.total_amount ?? baseFee : baseFee;
    const supplyAmount = isGitax ? application.supply_amount ?? application.total_amount ?? finalPaymentAmount : baseFee;
