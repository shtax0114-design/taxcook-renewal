(function () {
  ...
  const getBizNumber = (form) => ["biznum1", "biznum2", "biznum3"].map((name) => core.onlyDigits(getField(form, name))).filter(Boolean).join("-");
  ...
  const buildApplication = (form, formType, user) => {
    ...
    const type = isGitax ? "종합소득세" : "부가가치세";
    const processStatus = isGitax ? "신청 접수 및 계약금 결제" : "신청 접수";
  ...
  const fillFromProfile = (form, profile) => {
    ...
    if (profile.birth) {
      ...
      setValue("birthDate", day);
    }

    const bizSelect = form.querySelector("[data-biz-select]");
    if (bizSelect && profile.business_type) {
      bizSelect.value = profile.business_type;
  ...
  document.querySelectorAll(".renewal-apply-form").forEach((form) => {
    ...
    const syncBiznum = () => {
      ...
      const show = bizSelect.value === "사업자" || bizSelect.value === "간이사업자" || bizSelect.value === "일반사업자";
    ...
    form.querySelector("[name='sameMember']")?.addEventListener("change", async (event) => {
      ...
      try {
        ...
        fillFromProfile(form, profile || {});
      } catch (error) {
        event.target.checked = false;
        showAlert({ title: "로그인이 필요합니다", message: "회원 정보와 동일하게 입력하려면 먼저 로그인해주세요.", onClose: () => window.location.href = "login.html" });
      }
    });
    ...
    form.querySelector(".form-submit-preview")?.addEventListener("click", async (event) => {
      ...
      try {
        ...
        console.error(error);
        showAlert({ title: "신청 저장에 실패했습니다", message: "잠시 후 다시 시도해주세요. 문제가 계속되면 카카오채널로 문의해주세요." });
