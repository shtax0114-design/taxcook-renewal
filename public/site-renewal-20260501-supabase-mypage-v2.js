(function () {
  ...
  const renderApplications = (applications) => {
    ...
    applications.forEach((application) => {
      const values = [
        ...
        ["안내사항", application.customer_memo || application.process_status || "-"]
      ];
  ...
  const boot = async () => {
    ...
    try {
      ...
      fillText("[data-my-name]", profile?.name || core.emailToUserId(user.email) || "택스쿡 고객");
      fillText("[data-profile-userid]", profile?.user_id || core.emailToUserId(user.email));
      fillText("[data-profile-name]", profile?.name || "-");
      fillText("[data-profile-phone]", profile?.phone || "-");
      fillText("[data-profile-business]", profile?.business_type || "-");
      ...
      console.error(error);
      if (user) {
        fillText("[data-my-name]", core.emailToUserId(user.email) || "택스쿡 고객");
        fillText("[data-profile-userid]", core.emailToUserId(user.email));
      }
      renderApplications([]);
    }
  };
