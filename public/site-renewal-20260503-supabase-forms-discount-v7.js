(function () {
  ...
  const isVatRegularPeriod = (date = new Date()) => {
    const month = date.getMonth() + 1;
    const day = date.getDate();
    return (month === 1 && day >= 1 && day <= 25) || (month === 7 && day >= 1 && day <= 25);
  };

  const showVatPeriodGuide = () => {
    showAlert({
      title: "부가세 신고기간 안내",
  ...
  const normalizeCoupon = (value) => String(value || "").trim();
  ...
  const validateCoupon = async (coupon) => {
    ...
    const matched = (data || []).find((item) => String(item.code || "").trim().toLowerCase() === code.toLowerCase());
    if (!matched) {
      throw new Error("등록되지 않았거나 사용할 수 없는 할인코드입니다.");
    }
    return {
      code: matched.code,
      amount: toNumber(matched.amount)
    };
  };

  const buildApplication = (form, formType, user, discount = { code: "", amount: 0 }) => {
    const isGitax = formType === "gitax";
    const businessType = getField(form, "biz");
    const vataxSupplyFee = businessType.includes("일반") ? 100000 : 80000;
    const vataxFee = Math.round(vataxSupplyFee * 1.1);
    const discountAmount = Math.max(toNumber(discount.amount), 0);
    const vataxPayAmount = Math.round(Math.max(vataxSupplyFee - discountAmount, 0) * 1.1);
    ...
    return {
      ...
      coupon: discount.code || getField(form, "coupon"),
      ...
      base_fee: isGitax ? null : vataxFee,
      discount_amount: discountAmount,
      final_payment_amount: isGitax ? null : vataxPayAmount,
      total_amount: isGitax ? null : vataxPayAmount,
      supply_amount: isGitax ? null : vataxPayAmount
    };
  };
  ...
  const getBestProfile = async () => {
    ...
    if (stored?.id === user.id && (stored?.name || stored?.phone || stored?.business_type)) return stored;
    return core.getProfile(user);
  };

  const loginUrlForCurrentPage = () => {
    const hash = window.location.hash || "#apply-form";
    const returnTo = `${window.location.pathname}${window.location.search}${hash}`;
  ...
  document.querySelectorAll(".renewal-apply-form").forEach((form) => {
    ...
    const syncPeriod = (source) => {
      if (formType === "vatax" && source === regularCheck && regularCheck?.checked && !isVatRegularPeriod()) {
      ...
      try {
        ...
        if (!user) {
          return showAlert({
            ...
            onClose: () => window.location.href = loginUrlForCurrentPage()
          });
        }
        showAlert({
          title: "고객정보가 필요합니다",
    ...
    form.querySelector(".form-submit-preview")?.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (!user) {
          return showAlert({
            title: "로그인이 필요합니다",
            message: "신청을 접수하려면 먼저 로그인해주세요.",
            onClose: () => window.location.href = loginUrlForCurrentPage()
          });
        }
        const discount = await validateCoupon(getField(form, "coupon"));
        const application = buildApplication(form, formType, user, discount);
        ...
        showAlert({
          title: "신청 저장에 실패했습니다",
          message: error.message || "잠시 후 다시 시도해주세요. 문제가 계속되면 카카오채널로 문의해주세요."
        });
