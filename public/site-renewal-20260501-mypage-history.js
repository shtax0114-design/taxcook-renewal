...
const renderApplications = (profile) => {
  ...
  if (!applications.length) {
    body.innerHTML = '<tr><td colspan="5" class="empty-cell">아직 신청 내역이 없습니다.</td></tr>';
    return;
  }
  ...
  applications.forEach((application) => {
    ...
    [
      application.period || "-",
      formatDateTime(application.requestedAt || application.createdAt || application.updatedAt),
      application.reportedAt ? formatDateTime(application.reportedAt) : "-",
      application.paymentSummary || application.paymentStatus || "-",
      application.customerMemo || "-"
    ].forEach((value) => {
