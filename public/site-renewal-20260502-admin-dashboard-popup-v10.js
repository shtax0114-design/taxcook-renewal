(function () {
  ...
  let ownerPasscode = "";
  const protectedOwnerEmail = "chaewoon83@gmail.com";
  ...
  const renderMembers = () => {
    ...
    const filtered = members.filter((item) => {
      ...
      if (memberRoleFilter && role !== memberRoleFilter) return false;
      if (!keyword) return true;
      return [
        role,
        item.user_id,
    ...
    filtered.forEach((item) => {
      ...
      if (permissionMode && permissionUnlocked) {
        ...
        const isProtectedOwner = normalize(item.user_id) === protectedOwnerEmail;
        ...
        select.disabled = isProtectedOwner;
  ...
  const updateMemberRole = (event) => {
    ...
    if (!member) return;
    if (normalize(member.user_id) === protectedOwnerEmail) {
      select.value = member.role || "admin";
      setPermissionMessage("대표 계정은 권한을 변경할 수 없습니다.");
      return;
    }
  ...
  const saveMemberRoles = async () => {
    ...
    const adminCount = Array.from(nextRoles.values()).filter((role) => role === "admin").length;
    if (adminCount < 1) {
      setPermissionMessage("관리자는 최소 1명 이상이어야 합니다.");
      return;
    }
    const saveButton = document.querySelector("[data-permission-save]");
