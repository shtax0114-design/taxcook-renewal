...
-- Keeps application update/delete audit logs and exposes them to developer accounts only.
...
begin
  ...
  if tg_op = 'UPDATE' then
    ...
    from (
      ...
      join jsonb_each(to_jsonb(old)) as old_field on old_field.key = new_field.key
...
for each row execute function public.log_application_edit();
